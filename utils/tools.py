...
def visual_mse(true, preds, score, label, name):
    """
    Results visualization
    """
    ...
    if preds is not None:
        plt.plot(preds, label='Prediction', c='dodgerblue', linewidth=2)
    plt.plot(true, label='GroundTruth', c='tomato', linewidth=2)
    plt.plot(score, label='Anomaly Score', c='green', linewidth=2)
    plt.plot(label, label='Anomaly Label', c='black', linewidth=2)
    plt.legend(loc='upper left')
    plt.savefig(name, bbox_inches='tight')
