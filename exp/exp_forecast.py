        ...
        with torch.no_grad():
            for output_ptr in range(len(self.args.output_len_list)):
                ...
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                    ...
                    dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                    dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
                    inference_steps = self.args.output_len // self.args.pred_len  #预测步数，确定是否要滚动预测
                    ...
                    for j in range(inference_steps):
                        if len(pred_y) != 0:
                            #将先前预测的结果 pred_y[-1] 添加到 batch_x 的末尾，形成新的输入序列。batch_x 的维度保持 [batch_size, seq_len, num_features]。
                            batch_x = torch.cat([batch_x[:, self.args.pred_len:, :], pred_y[-1]], dim=1) 
                            tmp = batch_y_mark[:, j - 1:j, :]
                            batch_x_mark = torch.cat([batch_x_mark[:, 1:, :], tmp], dim=1) #确保时间戳信息的正确性
